...
### Added
- `Field\IblockSectionElementSelect` — выбор разделов и элементов одного инфоблока в общем упорядоченном списке: один диалог `ui.entity-selector` с вкладками «Разделы» и «Элементы», порядок задаётся перетаскиванием чипсов. Значения хранятся с префиксом типа (`s:<id>` — раздел, `e:<id>` — элемент, голое число — элемент для обратной совместимости), статический `decode()` разбирает их в типизированный список без обращения к БД. Заголовки вкладок настраиваются `tabsTitles()`.

  Класс приходит из проектной практики: до этого релиза он жил только как правка внутри `vendor/` у потребителя, поэтому любой `composer install` пакета удалял его и ронял страницы настроек фаталом `class not found`. Публичный API совпадает с той версией — потребителям достаточно удалить локальную копию из `vendor/`.

  Разделы и элементы грузятся статически (по одному запросу на тип), без постраничного догружения: поле рассчитано на небольшие инфоблоки. Документация — `docs/user/reference/fields/iblock-section-element-select.md`.

## [0.1.13]

### Fixed
- `DialogSelect` и наследники (`IblockElementSelect`, `IblockSectionSelect`, `IblockSectionElementSelect`, …) — сохранённое значение больше не удваивается при каждом сохранении формы. Скрытые инпуты значения наполнялись из двух независимых источников: `DialogSelector` перебирал `dialog.selectedItems` при инициализации, и дополнительно обработчик `onTagAdd` срабатывал на те же предвыбранные теги, которые `TagSelector` восстанавливает из этих же selectedItems. `ValueItemCollection.add()` дедупликации не имел, поэтому список значений рос по степеням двойки (2 → 4 → 8 записей на объект), а админка при отрисовке чипов дедуплицирует и проблему не показывала. Значение чинится само при первом сохранении формы после обновления.
- `ValueItemCollection::get()` — метод не работал вовсе: `forEach` с `return` внутри всегда возвращал `null`, а условие сравнивало метод `getValue` со значением вместо его вызова.
- `ValueItemCollection::delete()` — `splice()` внутри `forEach` сдвигал индексы и пропускал элементы; переписано на `filter()`.
- Значения сравниваются приведёнными к строке: из `dialog.selectedItems` приходит `id` сущности, из `onTagAdd` — `tag.getId()`, и для числовых ID это были число против строки.

## [0.1.10]

### Added
- `Field\Json` — управление раскладкой подполей. `->stacked()` (алиасы `->vertical()`, `->layout(Json::LAYOUT_STACK)`) выкладывает подполя вертикально, друг под другом, каждое со своим инлайн-лейблом; по умолчанию сохраняется прежняя горизонтальная раскладка (`Json::LAYOUT_ROW`) с общей шапкой. В списочном режиме (`multiple`) каждая строка становится самостоятельной карточкой с тулбаром (ручка перетаскивания + удаление), в одиночном (`multiple(false)`) — вертикальным стеком. Контракт POST-имён (`col[i][sub]` / `col[sub]`), sortable-drag и клиентский шаблон добавления строк не изменились. Добавлен геттер `->isStacked()`.
- `Field\Json` — вложенная раскладка прямо в `fields()`: рядом с обычными полями можно передавать группы `Field\Layout\Row::make([...])` — поля внутри `Row` встают в одну горизонтальную строку (ширины колонок берутся из `->width()`), а верхнеуровневые записи по-прежнему идут стеком. Это позволяет смешивать раскладки (часть полей в ряд, часть — по одному). Любой `Row` в схеме включает вертикальный режим; хранимое значение и контракт данных не меняются (плоский список листовых полей).
