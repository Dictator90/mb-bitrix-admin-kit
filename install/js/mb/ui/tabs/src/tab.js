...
export class Tab extends EventEmitter
{
	...
	#initHead(headOptions: TabHeadOptionsType | HTMLElement | string): HTMLElement
	{
		...
		{
			innerHeader = this.#buildHeaderInner(options);
		}
		else
		{
	...
	#buildHeaderInner(options: TabHeadOptionsType): HTMLElement
	{
		const hasIcon = Type.isStringFilled(options.icon);
		const hasCount = Type.isNumber(options.count) || Type.isStringFilled(options.count);

		const titleEl = Tag.render`<span class="ui-tabs__tab-header-title" title="${Text.encode(options.description ?? '')}">${Text.encode(options.title) ?? '&nbsp;'}</span>`;

		const iconEl = hasIcon
			? Tag.render`<span class="ui-tabs__tab-header-icon ui-icon-set ${Text.encode(options.icon)}"></span>`
			: null;

		const countEl = hasCount
			? Tag.render`<span class="ui-tabs__tab-header-count">${Text.encode(String(options.count))}</span>`
			: null;

		const inner = Tag.render`<span class="ui-tabs__tab-header-inner"></span>`;

		if (iconEl) Dom.append(iconEl, inner);
		Dom.append(titleEl, inner);
		if (countEl) Dom.append(countEl, inner);
