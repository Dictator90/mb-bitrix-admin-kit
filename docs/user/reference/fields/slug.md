...
Да. `from(...)` — явный способ указать источники для slug. Вызов `dependsOn('COLUMN')` **без callback** делает то же самое: колонка добавляется в источники и slug пересчитывается реактивно (`adminkit_action=reactive`).

При необходимости можно добавлять и свои зависимости с callback:
