...
namespace {
    ...
    $GLOBALS['MB_ADMIN_KIT_BITRIX_MAIN_LIB'] = $bitrixMainLib;
}
        ...
        class Application
        {
                ...
                return self::$connection ??= new class () {
                    ...
                    public function getSqlHelper(): object
                    {
                        return new class () {
                            ...
                            public function getDatetimeToDateFunction(string $value): string
                            {
                                return 'DATE(' . $value . ')';
                            }
                        };
            ...
            public function getCache(): object
            {
                return new class () {
                    ...
                    public function startDataCache(): void
                    {
                    }
            ...
            public function getTaggedCache(): object
            {
                return new class () {
...
namespace Bitrix\Main\ORM\Fields\Relations {
    ...
    if (!($GLOBALS['MB_ADMIN_KIT_BITRIX_ORM_AVAILABLE'] ?? false) && !class_exists(Reference::class)) {
        class Reference
        {
            public function __construct(
                public string $name,
                public string $referenceEntity = '',
                public array $referenceFilter = [],
            ) {
            }
        }
    }
...
namespace Bitrix\Main\ORM\Fields {
    ...
    if (!($GLOBALS['MB_ADMIN_KIT_BITRIX_ORM_AVAILABLE'] ?? false) && !class_exists(ExpressionField::class)) {
    ...
    if (!class_exists(Loc::class)) {
        class Loc
        {
            ...
            public static function loadMessages(string $file): void
            {
                ...
                foreach (['ru'] as $lang) {
                    $relativeFromSrc = str_replace(
                        '\\',
                        '/',
                        preg_replace('#^.*/src/#', 'src/', $relative) ?? $relative
                    );
...
namespace {
    ...
    if (($GLOBALS['MB_ADMIN_KIT_BITRIX_ORM_AVAILABLE'] ?? false) && is_string($GLOBALS['MB_ADMIN_KIT_BITRIX_MAIN_LIB'] ?? null)) {
        $bitrixMainLib = $GLOBALS['MB_ADMIN_KIT_BITRIX_MAIN_LIB'];

        $blockedBitrixClasses = [
        ...
        spl_autoload_register(static function (string $class) use ($bitrixMainLib, $blockedBitrixClasses): void {
            if (!str_starts_with($class, 'Bitrix\\Main\\')) {
                return;
            }
            ...
            if (in_array($class, $blockedBitrixClasses, true)) {
                return;
            }

            if (str_starts_with($class, 'Bitrix\\Main\\ORM\\')) {
                $path = str_replace('\\', '/', substr($class, strlen('Bitrix\\Main\\ORM\\')));
                $base = $bitrixMainLib . '/orm/';
            } else {
                $path = str_replace('\\', '/', substr($class, strlen('Bitrix\\Main\\')));
                $base = $bitrixMainLib . '/';
            }

            foreach ([strtolower($path), $path] as $candidate) {
                $file = $base . $candidate . '.php';
                if (is_file($file)) {
                    require $file;
                    return;
                }
        ...
        spl_autoload_register(static function (string $class): void {
            ...
            if (class_exists(\Bitrix\Main\ORM\Loader::class, false)) {
                \Bitrix\Main\ORM\Loader::autoLoad($class);
            }
        });
    }
}
